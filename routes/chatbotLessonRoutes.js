...
const AviationService = require("../services/lessonService");
const LoggingService = require("../services/loggingLessonService");
const {generateSystemMessage} = require("../services/systemMessageLesson");

router.post("/", async (req, res) => {
  const startTime = Date.now();
  try {
    const { message, page_id = null } = req.body;
    const topic_lesson_id = page_id;
    ...
    const currectLessons = await AviationService.currectLessons(topic_lesson_id);

    const conversationHistory = await  LoggingService.getConversationHistory(userId,5)


    const systemMessage =  generateSystemMessage(explanations, currectLessons, conversationHistory);
...
router.put("/logs/:logId", async (req, res) => {
  try {
    ...
    const { newMessage, page_id } = req.body;
    ...
    const currectQuestions = await AviationService.currectLessons(topic_lesson_id);
