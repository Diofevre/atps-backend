...
class LoggingService {
  static async logInteraction({
    ...
    try {
      ...
      const [rows] = await pool.execute(
        "SELECT updated_at, id FROM chatbot_logs WHERE id = " + insertedId,
      );

      if (rows.length > 0) {
        const dateCreated = new Date(rows[0].updated_at); // Convertit en objet Date
  ...
  static async getLogs(userId, limit = 100) {
    try {
      const [logs] = await pool.execute(
        "SELECT * FROM chatbot_logs WHERE user_id = '" + userId + "' ORDER BY created_at DESC LIMIT "+limit,
      );
  ...
  static async getLogById(logId) {
    try {
      const [rows] = await pool.execute(
        "SELECT * FROM chatbot_logs WHERE id = ?",
        [logId]
      );
      return rows[0] || null;
    } catch (error) {
  ...
  static async updateUserMessageAndResponse(
    ...
    try {
      ...
      const [rows] = await pool.execute(
        "SELECT updated_at FROM chatbot_logs WHERE id = ?",
        [logId]
      );
  
      if (rows.length > 0) {
        const updatedAt = new Date(rows[0].updated_at); // Convertit en objet Date
        return {
          success: true,
  ...
  static async deleteLog(logId, userId) {
    try {
      ...
      const [rows] = await pool.execute(
        "SELECT id FROM chatbot_logs WHERE id = "+ logId +" AND user_id = '"+ userId +"'",
      );
      ...
      // Supprimer le log
      await pool.execute("DELETE FROM chatbot_logs WHERE id = "+logId);
